   ...
   if(isset($_POST['submit'])) {
    ...
    switch($action){
      case "upd":
          ...
          CallInsertJournal($IDUSER,64,$LOGIN." a modifié Type machine '".$design."' (".$genre."/".$statut.")");
          }
          break;
      case "add":
          if ($design){
          ...
          CallInsertJournal($IDUSER,64,$LOGIN." a ajouté Type machine '".$design."' (".$genre."/".$statut.")");
          }
          break;
      case "del":
          if ($id){  
          CallDeleteTypeMachine($id);
          echo "<div class='tempmodal green'>Vos données ont été effacées</div>";
          CallInsertJournal($IDUSER,64,$LOGIN." a effacé Type machine '".$design."' (".$genre."/".$statut.")");
          break;
...
echo "<div class='titrepage'>Types de Machines</div>";
...
<script>
 ...
 if(hps.rows[index]){id = hps.rows[index].id;if (id){  document.getElementById("hpi").value= id;
  ...
  p2 = hps.rows[index].cells[0].getAttribute('data-sta');document.getElementById("hp0").value= p0;document.getElementById("hp1").value= p1; document.getElementById("hp2").value= p2;}}},false);
</script>