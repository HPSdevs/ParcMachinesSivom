...
//----traitement
   if(isset($_POST['submit'])) {
    ...
    switch($action){
      case "upd":
          if ($id){
          CallUpdateModeleMachine($design,$genre,$statut,$id);
          echo "<div class='tempmodal green'>Vos données ont été sauvegardées</div>";
          CallInsertJournal($IDUSER,64,$LOGIN." a modifié Modele machine '".$design."' (".$genre."/".$statut.")");
          }
          break;
      case "add":
          if ($design){
          CallInsertModeleMachine($design,$genre,$statut);  
          echo "<div class='tempmodal green'>Vos données ont été ajoutées</div>";
          CallInsertJournal($IDUSER,64,$LOGIN." a ajouté Modele machine '".$design."' (".$genre."/".$statut.")");
          }
          break;
      case "del":
          if ($id){  
          CallDeleteModeleMachine($id);
          echo "<div class='tempmodal green'>Vos données ont été effacées</div>";
          CallInsertJournal($IDUSER,64,$LOGIN." a effacé Modele machine '".$design."' (".$genre."/".$statut.")");
          }
          break;
...
foreach ( $modelemachines as $m){
  echo"<tr id='".$m['id_modelemachine']."'>";
